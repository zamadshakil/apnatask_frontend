...
export default function CanonicalProviderTabs() {
  return <View style={styles.root}><Tabs tabBar={(props) => <View><ActiveTaskDock role="provider" /><GlassTabBar {...props} /></View>} screenOptions={{ headerShown: false, sceneStyle: { backgroundColor: Theme.colors.background }, tabBarHideOnKeyboard: true }}><Tabs.Screen name="index" options={{ title: i18n.t('tabs.jobs'), tabBarIcon: ({ color }) => <BriefcaseBusiness color={color} /> }} /><Tabs.Screen name="assigned" options={{ title: i18n.t('experience.assigned.title'), tabBarIcon: ({ color }) => <ClipboardCheck color={color} /> }} /><Tabs.Screen name="credits" options={{ title: i18n.t('tabs.credits'), tabBarIcon: ({ color }) => <Coins color={color} /> }} /><Tabs.Screen name="account" options={{ title: i18n.t('tabs.account'), tabBarIcon: ({ color }) => <UserRound color={color} /> }} /></Tabs></View>;
}
